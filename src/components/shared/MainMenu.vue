<template>
  <nav id="mainMenu">
    ...
    <ul class="menu-group" @click.stop="toggleModal($event)">
      <li class="relative noselect" v-for="(item, id) in menu" :key="id">
        <div @mouseenter="modalNumber = id" class="center">{{item.title}}</div>
        <ul class="modal" :class="{hidden: id !== modalNumber}" v-show="modal">
          <li
    ...
    <div class="window-icons-group">
      <button class="fixed-width" @click.prevent="buttonHandler_minimize">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
          <path d="M19 13H5v-2h14v2z" />
          <path d="M0 0h24v24H0z" fill="none" />
        </svg>
      </button>
      <button class="fixed-width" @click.prevent="buttonHandler_maximize">
        <svg
      ...
      <button class="red fixed-width" @click.prevent="buttonHandler_quit">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
          <path
